...
function MestraAjaxLog (){
	...
	//determina a quantidade limite para que os debus sejam impressos no console, mas este só funcionará se isEnabledServidor = true
	this.limiteMsg = 5;
}
...
///################################################################################
/// método que efetua requisicao via ajax para logar no console#### Felipe Reis ###
...
///###############################################################
/// método que efetua o debug com alert ######### Felipe Reis ####
// - ex = excessao
// - msg = mensagem que será exibida para usuario
...
///############################################################
///método que efetua o debug via console ###### Felipe Reis ###
...
///###################################################################################################
///método que trata o resultado da requisicao feita pelo debug via console ####### Felipe Reis #######
...
{
    alert("Desculpe,a requisição expirou ! Tente novamente.");
}
...
//####### disparaAjax #########################################################################################################################################################
// Método que iniia o ajax disparado de acordo com os parametros passados
/* - strhostComp = host/nome do ActionMapping da rotina vigente 
   ...
   - metodoRespota = nome do método em que será tratado a resposta obtida via ajax */
//#############################################################################################################################################################################
...
// Método que obtem a resposta do servidor,processa e redireciona para o método de tratamento informado por parametro
/* - metodoRespota = nome do método em que será tratado a resposta obtida via ajax */ 
//###################################################################################################################
function processaRespostaAjax(metodoRespota, objhttp)
{
   ...
   {    
	   	if (xmlhttp.status == 200) // resposta do servidor OK
	    {
			...
			try{
				mestraAjaxLog.alertaMestraAjax('','MÉTODO DE RESPOSTA: '+metodoRespota,'D');
...
//####### eventoExecutaAjax ###########################################################################################################################################
//Método que chama o disparaAjax e recebe como parametros as informações necessárias para executar a operação em ajax
/* - id = chave em que será passados as propriedade do evento que será executa via ajax (ex: recuperaPlano.nomeAction,recuperaPlano.nomeAction,recuperaPlano.parametros
   - metodoRespota = nome do método em que será tratado a resposta obtida via ajax 
...
//###################################################################################################################################
//Método inicial do ajax onde irá comecar tudo, este que será chamado onde for preciso inserir ajax ######### Felipe Reis ###########
// campo = passe o objeto (this)
// id = identificador unico do evento a ser executado (ex: eventoDisparadoPeloFundo)
// metodoRespota = metodo em que o ajax retornara o resultado requisitado, ou seja, onde haverá o tratamento da string de resultados
//##################################################################################################################################
...
//####################################################################################################################################
//Método que axecuta todos os metodos de tratamento de campo dependente que foram adicionados no array ARexecutarAjaxCampoDependente
...
function trataCampoDependente(nomeCampo){
	//Executa Métodos ARexecutarAjaxCampoDependente
...
//########################################################################################################
//Método que executa todas as chamadas de ajax adicionadas no array ARexecutarAjaxOutrosEventos
// id = identificador do metodo que sera executado, ou seja, o proprio nome do metodo ### Felipe Reis ####
//########################################################################################################
function executarAjax(id){

	//Executa Métodos ARexecutarAjaxOutrosEventos
...
//###########################################################################################################################################################################
//Método que adiciona funcoes para serem executadas dentro de executarAjax ########## Felipe Reis ###########################################################################
// nomeFuncaoRecuperaParametros = nome da funcao para ser adicionada no bloco de funções que sao responsaveis por inicializar os parametros necessario de uma chamada de ajax
//###########################################################################################################################################################################
...
//################################################################################################################################
//Método que adiciona funcoes para serem executadas dentro de trataCampoDependente ############# Felipe Reis #####################
// trataCampoDependente = nome da funcao para ser adicionada no bloco de funções que sao responsaveis tratar os campos dependentes
//################################################################################################################################
...
// ## Criar a seguinte variável a página de utilização do ajax:
...
// ## Exemplo para chamada do método no onclick='mostrarAnimaAjax(event);' 
// ## @Updated Felipe Reis
///#########################################################################
...
//Caminho da imagem que representa o ícone do ajax
...
//#####################################
//Método que inicia a animacao do ajax
//#####################################
...
//############################################################################################
// ######## Felipe Reis ######################################################################
// Recupera a posicao do campo na tela, para que o ícone do ajax seja mostrado ao lado deste.
// obj = campo da tela (this)
//############################################################################################
...
///############################################################
// ######## Fim Animação do ajax #############
///############################################################

//########################################################################################################################################################################
// ######## Felipe Reis 12/09/2008 12:58:30
	/* Método genérico para popular combos simples com código+descricao por ajax.
	Parâmetros:
	...
	- idValue = informe qual a posicao das strings inseridas na lista de resultados que será o VALUE da combo.
	- idCodigo = informe qual a posicao das strings inseridas na lista de resultados que será o CÓDIGO DE VISUALIZAÇÃO ( CD - DC ) da combo.
	- idDescricao = informe qual a posicao das strings inseridas na lista de resultados que será o DESCRIÇÃO DE VISUALIZAÇÃO ( CD - DC ) da combo.*/
//##########################################################################################################################################################################
...
// ######## Felipe Reis 12/09/2008 12:58:45
	/* Método genérico para popular combos simples com código+descricao por ajax com detalhe.
	Parâmetros:
...
//########################################################################################################
// ######## Felipe Reis 12/09/2008 12:58:45
/* Método responsável por associar os parametros necessários para o evento que deve ser executado via ajax
   - id = identificador do Evento
   - nomeAction = nome da action da rotina vigente
   - nomeEvento = nome do evento à executar 
...
//########################################################
// ######## Felipe Reis 12/09/2008 12:58:45
/* Método responsável por remover ítens de combos simples
   - campo = nome da combo que deseja removê-la */
//#########################################################
...
//##################################################################
// ######## Felipe Reis 12/09/2008 12:58:45
/* Método responsável por remover ítens de combos detro de detalhes
   - nomeLista = nome da lista em que a combo se encontra 
   - campo = nome da combo que deseja removê-lo */
//##################################################################
...
//##############################################################
// ######## Felipe Reis 12/09/2008 12:58:45
/* Método responsável por remover ítens do form com lista 
   - nomeLista = nome da lista em que a combo se encontra 
   - nomeCampo = nome da combo que deseja removê-lo 
   - posicaoNoForm = posicão da lista em que deseja removê-lo */
//###############################################################
...
//######################################################
// ######## Felipe Reis 12/09/2008 12:58:45
/* Método responsável por remover ítens do form simples
   - campo = nome do campo que deseja removê-lo */
//######################################################
...
//#########################################################
// ######## Felipe Reis 12/09/2008 12:58:45
/* Método responsável por popular um campo do form simples
...
//############################################################
// ######## Felipe Reis 12/09/2008 12:58:45
/* Método responsável por popular um campo do form com lista
   - nomeLista = nome da lista em que a combo se encontra 
...
/** Método responsável por remover elementos de uma combo
   - combo = nome da combo que deseja remover o elemtento
   - posicao = posicao do elemento na combo (ex: 0 .. 1 ou 2 ..)
   - texto = texto para comparacao e verificacao se o elemento da posicao informada é o desejado */
//##################################################################################################
