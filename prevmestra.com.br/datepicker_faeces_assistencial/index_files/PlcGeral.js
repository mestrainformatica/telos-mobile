/**
 * Arquivo geral de funções javascript do jCompany.
...
//if (NNav) document.captureEvents(Event.KEYDOWN)


/**
 * Função para executar funções no onload da página
 * @see setFocus
...
/**
 * Função posicionar foco em campo específico
 * @param campo Campo a ser focado [String]
 * @variable campoFocus Variável que identifica campo a ser posicionado
 */
var campoFocus = "";
...
/**
 * Função dispara clique em um botão da página conforme acao passada
 * @param acao Ação que determina botão de exclusão a ser disparado [String]
 * @return disparou Tipo boolean que informa se o botão foi disparado ou não
 */
...
/**
 * Função de alerta em janela com mensagem<br>
 * Passo 1: Colar as linha abaixo no fim da página:<br><dd>
 *	<blockquote><code>&lt;!-- INÍCIO DIV DE ALERTA --&gt;<br>
 ...
 *  @param evt Não utilizado (RETIRAR)
 *  @see showDiv
 */
...
/**
* Função que chama janela de ajuda
...
/**
 * Função que mostra/esconde div de mensagem
 *  @param div Nome do div onde a mensagem vai ser mostrada [String]
 *  @param show Variável que indica define se o div será mostrado ou escondido [boolean] 
 */
...
* Funcionamento:
* Função de foco no primeiro campo existente na página.
* Após o onload da página a função procura nos campos da página que não são readonly
* o primeiro campo para foco da seguinte forma:
* campos text, password e textarea que não estejam preenchidos
* campos combobox e listbox que não estejam vazios e nem com algum valor selecionado
*/
...
/**
* Função de registro de avisos on-line
...
/**
* Função para testar se existe mensagem online
* @variable avisoArray Array onde são guardados os dados do aviso on-line
*/
...
/**
* Função para testar se existe mensagem online
* @variable avisoArray Array de onde são retiradas os dados do aviso on-line
* @see janela
*/
...
/**
* Função que altera o CLASS de um elemento
...
/**
* Função para guardar dados de impressão
* @param url Endereço para abertura da janela de impressão
* @param titulo Título da janela. [String]
* @param nome Nome da janela. [String]
* @param html Conteúdo para impressão. [String]
*/
var objImpressao;
...
* @param titulo Título da janela. [String]
* @param nome Nome da janela. [String]
* @variable objImpressao Objeto que contém dados para impressão
* @see objetoImpressao
* @see htmlImpressao
*/
...
* A função retira do parâmetro objeto uma parte do html entre as<br>
* tags de comentário e retorna este html para a página que a chamou
* Chamada: <br><dd><code>&lt;a href='#' onclick='janela("url_janela","width","height","props"); return false;'&gt;</code>
...
{
	//Objeto => parâmetro que representa uma página
...
/**
* Função que redireciona página estática fora do projeto (abrindo em nova instância)
* Chamada: &lt;body onLoad=redireciona("URL_DA_PÁGINA");&gt;
* @param url Endereço para redirecionamento
*/
...
/**
* Função para exibir uma mensagem de texto como alerta
...
/**
* Funções para registro de botões que respondem a eventos
...
/**
* Funções para capturar tecla pressionada e executar ação associada à tecla
* @param nomeBotao Nome do botão que atenderá ao evento
* @param evento Evento associado ao botão
* @variable disparouBotao Indicador de botão disparado
* @variable disparouEnter Indicador de tecla ENTER pressionada
* @variable interval Intervalo de tempo utilizado para execução de ação<br> 
*			associada à tecla ENTER
...
/**
* Funções para identificar a tecla pressionada
...
/**
* Funções que seleciona o botão associado à função da tecla
* @param acao Ação à qual o botão está associado
* @return botao Objeto Button representando o botão associado à ação
*/
...
*/
//FUNÇÃO PARA SETAR AÇÃO DO BOTÃO PRESSIONADO
var botaoAcao = "";
...
*/
//FUNÇÃO PARA SETAR AÇÃO DO BOTÃO AO PRESSIONAR ENTER
var botaoAcaoEnter = "";
...
* Os parâmetros devem ser passados para este função aos pares e na seguinte ordem: nome e valor do 
* atributo.
* @see recebeSelecao
...
function devolveSelecaoPopup(listaValores)
{
	...
	{
		separa = listaValores.split("'");
		vFinal = "";
		for(var i = 0; i < separa.length; i++) {
			vFinal = vFinal + separa[i];
		}
		
		opener.recebeSelecaoPopup(vFinal);
		window.close();
	}
}
...
function devolveSelecao()
{
	...
	{	
		// Verifica se o número de argumentos é par
		if ((arguments.length % 2) != 0 ) {
			alert("ERRO! \n\nFunção: devolveSelecao(). \n\nErro: A quantidade de parametros passados para esta função não é um número par." );
			return;
		}
...
/******************************************************************************
*	FUNÇÃO QUE BLOQUEIA DIGITAÇÃO DE CARACTERES NÃO PERMITIDOS	
* 
* ------------------------------------------------
* Função:		validaCaracter(evt, tipo)
* ------------------------------------------------
*
*=> campo  =	Tipo: String
*		Nome do campo atual
...
* FORMATO: 	regEvento ("Evento","Ação Executada","Funcao Associada à Ação");
* ACÕES:		TESTE:		Executa alguma função (deve-se informar uma função para teste)
*				MENSAGEM:	Envia uma mensagem
*
* EVENTOS:  Evento podem ser padrões (definidos pelas ações dos botões de ação) ou definidos
*				pelo programador.
* Ex.: 
*		Evento 'F10-Gravar':  ocorre quando o usuário executa gravação de algum registro
*		Evento 'TESTAR':  	 deve ser executado pela chamada da função 
...
/************************************************************************
* 		LÓGICAS PARA REGISTRO E ENVIO DE MENSAGENS DE ALERTA GENÉRICAS
*
...
*
************************************************************************/
//Array que guarda mensagens da aplicação
...
//Função que seta os valores retornados nos campos registrados para retorno
...
*/
/******************************************************************************
* LÓGICAS PARA MONTAR ARRAY DE CAMPOS E RETORNAR VALORES NESTES CAMPOS
******************************************************************************/
...
/************************************************************************
* 			FUNÇÕES PARA SETAR VALOR EM CAMPO INFORMADO
************************************************************************/
...
/************************************************************************
* 			FUNÇÃO PARA SETAR FOCUS EM UM CAMPO INFORMADO
************************************************************************/
var botao;
...
/************************************************************************
* 			FUNÇÕES PARA TRATAMENTO DE RETORNO DE DATA DO CALENDÁRIO
************************************************************************/
//Campo para retorno de data
var campoData;
//Função para abertura do calendário
...
//Função para retorno da data no campo indicado
...
/**********************************************************************************
* 					FUNÇÃO PARA RETORNAR UM VALOR EM UM CAMPO
**********************************************************************************/
...
/**********************************************************************************
* 					FUNÇÃO PARA INSERIR UM VALOR EM UM CAMPO
**********************************************************************************/
...
/**********************************************************************************
* 					FUNÇÃO PARA REDIRECIONAMENTO DE ENDEREÇO
**********************************************************************************/
...
/******************AVALIAÇÃO DE EXPRESSÕES COM REPETIÇÃO DE CAMPOS ***************
=> Avalia a expressão passada
=> Se informado um campo para retorno este irá receber o valor da expresssão 
avaliada.
=> Sintaxe: avaliaExpressao(expressao, campoRetorno)
=> Parâmetros ( O = Obrigatório)
	expressao 	= expressão para ser avaliada
		Para a composição da expressão podem ser utilizado algarismos, operadores,
		parênteses e campos da aplicação.
		Quando for utilizar campos informar o nome do campo entre "#". 
		Exemplo: ((10-20)*5+#campo1#)/campo2
	campoRetorno= campo para onde será enviado o valor da expressão avaliada 
*********************************************************************************/
...
function avaliaExpressao(expressao, campoRetorno)
{
	...
	{
		alert("Expressão não pode conter espaços.");
		return;
	}
	else
	{
...
/**
* 				FUNÇÕES PARA TORNAR VISÍVEIS/INVISÍVEIS CAMPOS DO FORM
*/
...
/**********************************************************************************
* 			FUNÇÃO PARA CONCATENAR DOIS VALORES COM O SEPARADOR INFORMADO
**********************************************************************************/
...
/**
* 			FUNÇÃO PARA SEPARAR UM LISTA DE TERMOS EM ARRAY
*/
...
/**********************************************************************************
* 					FUNÇÃO PARA CÁLCULO DE VALORES
**********************************************************************************/
...
/******************************************************************************
* FUNÇÃO QUE RETORNA UMA STRING PARA USO EM URL, CONTENDO O NOME DO CAMPO+VALOR DO CAMPO,
...
{
//	checarUm(checkbox);
/*	var oDiv = document.createElement("<DIV ID='DIV_"+divs.length+"' style=\"position:absolute; visibility:'visible'; width:450; height:1; left:"+(evt.x+20)+"; top:"+evt.y+" \"></DIV>");
	var oIMG = document.createElement("<IMG SRC='./plc/midia/linha_exclui.gif' border=0 height=1 width=400>");
	var div = "";
	div = getID(checkbox.name);
	if(checkbox.checked)
	{
		if(div == "")
		{
			...
			divs[divs.length] = new setID(checkbox.name, "DIV_"+divs.length);
		}
		else
...
/*	var pai 		= document.forms[0].elements[E.sourceIndex+cont];
	var filho 	= pai.children;
	if (filhos[i].tagName == tag)
	{
		var netos = filhos[i].children;
		var texto = netos[0].innerText;
		var arrTexto = separaListaTermos(texto, "");
		if (arrTexto.length > 1 && isNaN(parseInt(arrTexto[0])))
		{
				filhos[i].className 	= classe;
				filhos[i].marcado 	= true;
		}
	}
*/
...
/*	var pai = E.parentElement;
	...
	{
		if (filhos[i].tagName == tag && !filhos[i].marcado)
		{
			var netos = filhos[i].children;
			var texto = netos[0].innerText;
			var arrTexto = separaListaTermos(texto, "");
