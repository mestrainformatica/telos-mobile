	...
	var initializeMenus = function() {	
		if(($('#menuDiv').length > 0) && (!initializedMenu)){
			initializedMenu = true;
			// esconde os submenus
			fecharSubmenus();
			// ao clicar, abre o menu
			$('#menuList > li > a').click(function(){
				var $ulSubmenu = $(this).parent().find('ul:first');
				console.log($(this));
				if ($ulSubmenu.is(':visible')){
					$ulSubmenu.hide();
					this.style.backgroundImage = "url(/img/chevron-down.png)";
				} else {
					$ulSubmenu.show();
					this.style.backgroundImage = "url(/img/chevron-up.png)";
				}
			})
			// ativar link clicado
			$('#menuList li.menubar ul.menu li a').click(function(){
				$('#menuList li.menubar ul.menu li a').removeClass('highlight');
				$(this).addClass('highlight');
				if($(window).width() < 767){$('#conteudo').addClass('menu-collapsed');} else {
					$('#conteudo').removeClass('menu-collapsed');					
				}
			});	
		}
	}
	var abrirSubmenus = function(){
		$('#menuList > li ul').each(function(){
			$(this).show();
		})
	}
	var fecharSubmenus = function(){
		$('#menuList > li ul').each(function(){
			$(this).hide();
		})
	}
	...
	var createMenu = function(obj) {
		$('#menuList').empty();
		var html = '<li class="menu-title">Menu <span class="glyphicon glyphicon-menu-hamburger"></span></li>';
				
		jQuery.each(obj, function(i, val) {
			html += '<li class="menubar">';
			html += '<a href="javascript:void(0)" id="M' + val.codigo + 'Actuator" class="actuator">' + val.descricao + '</a>';
			if (val.itens) {
				  html += '<ul id="M' + val.codigo + 'Menu" class="menu" style="display: block;">';
				  jQuery.each(val.itens, function(j, item) {
					  html += '<li><a href="' + item.link + '">' + item.menu + '</a></li>';
				  });
				  html += '</ul>';
			 }
			 html += '</li>';
