...
/*
 * Criada/Modificada para aceitar caracteres alfanuméricos //Felipe Reis 03/06/2009 09:53h
*/
...
//######### Função para checkar varios checks juntos #######
 ...
 **/
 var moeda = {
	...
	formatar4CasasDecimais: function(num){
	   x = 0;
	   ...
	   if(isNaN(num)) num = "0";//num.toString().substring(num.toString().indexOf("."),num.toString().length)
	      cents = Math.floor((num*100+0.5)%100);

	   num = Math.floor((num*100+0.5)/100).toString();
	
	   if(cents < 10) cents = "0" + cents +"00"; else if(cents < 100) cents = cents + "00"; else if(cents > 100 && cents < 1000) cents = cents +"0";
	      for (var i = 0; i < Math.floor((num.length-(1+i))/3); i++)
	         num = num.substring(0,num.length-(4*i+3))+'.'
	               +num.substring(num.length-(4*i+3));
